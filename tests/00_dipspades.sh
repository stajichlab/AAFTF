...
if [ ! -f $LEFTTRIM ]; then
	../scripts/AAFTF trim --method fastp --left $OUTDIR/SRR5223785_1.fastq.gz --right $OUTDIR/SRR5223785_2.fastq.gz -o $OUTDIR/${PREFIX} -c $CPU --merge --dedup
fi
if [ ! -f $LEFT ]; then
 	../scripts/AAFTF filter --mem $MEM -c $CPU --left $LEFTTRIM --right $RIGHTTRIM --aligner bbduk -o $OUTDIR/${PREFIX}
	../scripts/AAFTF filter --mem $MEM -c $CPU --left $MERGED --aligner bbduk -o $OUTDIR/${PREFIX}
fi
if [[ -f $LEFT && -f $RIGHT ]]; then
	rm -f $LEFTTRIM $RIGHTTRIM $MERGEDTRIM
fi
exit
...
if [ ! -f $ASMFILE ]; then
	../scripts/AAFTF assemble --mem $MEM --left $LEFT --right $RIGHT --merged $MERGED --prefix $OUTDIR/${PREFIX} -c $CPU --method dipspades --workdir dipspades_run
fi
exit
if [ ! -f $VECCLEAN ]; then
